...
#### バリデーション・再試行システム（2025-09-02実装）:
- **厳密なバリデーション**: プレイヤー数一致・プレイヤー名妥当性・ランキング整合性を検証
- **自動再試行機能**: バリデーション失敗時に最大5回まで自動再生成
- **設定可能な再試行回数**: `processing.max_retries`で調整可能
- **詳細ログ出力**: 各試行の失敗理由と進捗を記録
...
### バリデーション詳細（2025-09-02実装）:
- **プレイヤー数チェック**: ランキング数がゲームのプレイヤー数と一致
- **プレイヤー名検証**: 全ての名前がキャラクター情報に存在
- **ランキング整合性**: 1からプレイヤー数までの連続した整数
- **重複排除**: ランキング値の重複を検出
- **自動再生成**: 設定可能な回数まで自動リトライ（デフォルト5回）
...
```json
{
  "team_averages": {
    "kanolab": {
      ...
      "ゲーム行動（投票、襲撃、占いなど）は対話内容を踏まえているか": 2.9,
      "発話表現は豊かか。与えられたプロフィールと矛盾なく、エージェントごとに一貫して豊かなキャラクター性が出ているか": 3.9
    }
  },
  "team_sample_counts": {
    "kanolab": {
      ...
      "ゲーム行動（投票、襲撃、占いなど）は対話内容を踏まえているか": 10,
      "発話表現は豊かか。与えられたプロフィールと矛盾なく、エージェントごとに一貫して豊かなキャラクター性が出ているか": 10
    }
  },
  "summary": {
    "total_games_processed": 14,
    "teams_found": ["kanolab", "GPTaku", "CamelliaDragons", "osawalab", "mille"],
    "criteria_evaluated": [
      "発話表現は自然か",
      "文脈を踏まえた対話は自然か",
      "発話内容は一貫しており矛盾がないか",
      "ゲーム行動（投票、襲撃、占いなど）は対話内容を踏まえているか",
      "発話表現は豊かか。与えられたプロフィールと矛盾なく、エージェントごとに一貫して豊かなキャラクター性が出ているか"
    ]
  }
}
```

**注意**: チーム集計結果では、評価基準の表示に`criteria_name`（例：`natural_expression`）ではなく、日本語の`description`（例：`発話表現は自然か`）が使用されます。この変換は`BatchProcessor`内で自動的に行われます。
...
- [x] **LLM評価エンジン実装**
  ...
  - [x] OpenAI GPT-4o統合
  - [x] Pydantic構造化出力検証
- [x] **バリデーション・再試行システム実装（2025-09-02）**
  - [x] 厳密なPydanticバリデーション（プレイヤー数・名前・ランキング整合性）
  - [x] 自動再試行機能（設定可能な回数まで）
  - [x] 詳細ログ出力（各試行の失敗理由と進捗）
  - [x] 設定ファイル連携（`processing.max_retries`）
- [x] **チーム情報管理の修正**
...
### LLMバリデーション・再試行システムの実装（2025-09-02）

#### 実装背景:
LLMが時々不完全または不正なランキングレスポンス（プレイヤー数不一致、存在しないプレイヤー名、ランキング重複等）を返すことが判明。信頼性の高い評価結果を確保するため、厳密なバリデーションと自動再試行機能を実装。

#### 実装内容:
1. **厳密なPydanticバリデーション**:
   - プレイヤー数一致チェック（ランキング数 == player_count）
   - プレイヤー名妥当性検証（全名前がcharacter_infoに存在）
   - ランキング整合性（1から連続した整数、重複なし）

2. **自動再試行メカニズム**:
   - バリデーション失敗時、設定可能な回数まで自動リトライ
   - 各試行で詳細ログ出力（warning/info/errorレベル）
   - 最終試行後も失敗の場合はValueErrorで処理停止

3. **設定ファイル統合**:
   ```yaml
   processing:
     max_retries: 5  # デフォルト5回まで再試行
   ```

#### 効果:
- **品質向上**: 不正なLLMレスポンスの自動修正
- **運用安定性**: 手動介入なしでの継続処理
- **設定柔軟性**: 環境に応じた再試行回数調整

### 評価基準ソート機能の実装（2025-09-03）

#### 実装背景:
チーム集計結果（`team_aggregation.json`）の出力時に評価基準の並び順が一定でなく、結果の可読性と一貫性に課題があった。そのため、評価基準ごとに順序を定義し、出力時に常に同じ順序で表示されるようにソート機能を実装。

#### 実装内容:
1. **evaluation_criteria.yamlへのorderフィールド追加**:
   ```yaml
   common_criteria:
     - name: "natural_expression"
       order: 1  # 発話の自然さ
...
3. **criteria_loaderでのorder読み込み対応**:
   - `order`フィールドを読み込み、デフォルト値0を設定
   - 既存の設定ファイルとの下位互換性を維持

4. **team_aggregation.json出力時のソート実装**:
   - `team_averages`および`team_sample_counts`の各チームデータをorderでソート
   - `summary.criteria_evaluated`もorderに基づいて並び替え
   - criteria_name → description変換後もソート順を保持

#### 効果:
- **出力一貫性**: チーム集計結果が常に同じ順序で出力
- **可読性向上**: 論理的順序（自然さ→対話→一貫性→行動→キャラクター→チームプレイ）での表示
- **設定柔軟性**: orderフィールドで評価基準の表示順を自由に調整可能
- **下位互換性**: 既存の設定ファイルでも正常動作（デフォルト値0で処理）
