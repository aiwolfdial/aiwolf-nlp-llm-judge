...
class BatchProcessor:
    ...
    def __init__(self, config: dict[str, Any], max_workers: int | None = None):
        """
        Args:
            config: 設定辞書
    ...
    def find_all_game_logs(self, input_dir: Path) -> list[AIWolfGameLog]:
    ...
    def process_all_games(self, input_dir: Path, output_dir: Path) -> None:
        """すべてのゲームログを並列処理で評価"""
        game_logs = self.find_all_game_logs(input_dir)
        ...
        logger.info(
            f"Starting batch processing of {len(game_logs)} games with {self.max_workers} workers"
        )
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for game_log in game_logs:
                future = executor.submit(
                    process_single_game, game_log, self.config, output_dir
                )
        ...
        logger.info(
            f"Batch processing completed. Success: {completed}, Failed: {failed}"
        )
